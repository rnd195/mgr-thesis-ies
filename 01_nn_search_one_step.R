## --------------------
## Description: Neural network models grid search 1h ahead
## Author: Martin Řanda
## Year: 2023
##
## R Version 4.2.2
...
for (j in 1:nrow(hyper_grid)) {
  ...
  for (epoch in 1:num_epochs) {
    ...
    # Early stopping mechanism - stop if validation MSE is 0.001 higher than previous
    # Note that once you choose a specification in the grid search, you should refit
    # the model with n - 1 epochs than those reported in the output table
  ...
  write.csv(na.omit(hyper_grid), "nn_hyper_grid_1h_n5.csv")
  # Clean up
  rm(net)
}
