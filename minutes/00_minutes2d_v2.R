## --------------------
## Description: Minute data analysis - 2 days (version 2)
## Author: Martin Řanda
## Year: 2023
##
## R Version 4.2.2
...
#### Prepare dates ####
starting <- seq.Date(as.Date("2011-01-01"), as.Date("2021-12-29"), by = "day") %>%
  as.character()
ending <- seq.Date(as.Date("2011-01-02"), as.Date("2021-12-30"), by = "day") %>%
  as.character()
oos_days <- seq.Date(as.Date("2011-01-03"), as.Date("2021-12-31"), by = "day") %>%
  as.character()
reg_days <- paste0(starting, "/", ending)


#### Run the models ####
all_oos <- paste0(oos_days[1], "/", oos_days[length(oos_days)])
fcst_series <- xts(rep(NA, nrow(series[all_oos])), order.by = index(series[all_oos]))

# Diagnostic tests lags
lags <- 4:7
cols <- c(
  "train_days", "oos_day", "spec", "fcst_rmse", "rw_rmse", "fcst_mae", "rw_mae", paste0("box_p_lag", lags), paste0("arch_p_lag", lags), "jb_p", "dm_p_1", "dm_p_2"
)
...
for (days in reg_days) {
  ...
  fcst_series[oos_days[j]] <- onesteps

  # Add diagnostics (Ljung-Box and ARCH test)
  boxtests <- rep(NA, length(lags))
  archtests <- rep(NA, length(lags))

  for (lag in lags) {
    boxtests[lag - 3] <- Box.test(mod$residuals, lag = lag, type = "Ljung-Box")$p.value
    archtests[lag - 3] <- Box.test(mod$residuals^2, lag = lag, type = "Box-Pierce")$p.value
  }
  jbtest <- JarqueBeraTest(mod$residuals)$p.value

  # Diebold-Mariano test, Forecast Error (Residual) = Actual − Predicted
  ...
  results_df[j, paste0("box_p_lag", lags)] <- boxtests
  results_df[j, paste0("arch_p_lag", lags)] <- archtests
  ...
  if ((j %% 10) == 0) write.csv(results_df, "results_df_2d_v3.csv")
}
write.csv(results_df, "results_df_2d_v3.csv")
colnames(fcst_series) <- "forecast"
saveRDS(fcst_series, "results_fcst_2d_v3.rds")
