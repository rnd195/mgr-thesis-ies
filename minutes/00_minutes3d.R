## --------------------
## Description: Minute data analysis - 3 days
## Author: Martin Řanda
## Year: 2023
##
## R Version 4.2.2
...
#### Prepare dates ####
starting <- seq.Date(as.Date("2011-01-01"), as.Date("2021-12-28"), by = "day") %>%
  as.character()
ending <- seq.Date(as.Date("2011-01-03"), as.Date("2021-12-30"), by = "day") %>%
  as.character()
oos_days <- seq.Date(as.Date("2011-01-04"), as.Date("2021-12-31"), by = "day") %>%
  as.character()
...
for (days in reg_days) {
  ...
  if ((j %% 10) == 0) write.csv(results_df, "results_df_3d_v2.csv")
}
write.csv(results_df, "results_df_3d_v2.csv")
